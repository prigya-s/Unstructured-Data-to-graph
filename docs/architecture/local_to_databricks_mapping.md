...
| Local embeddings via `OllamaEmbeddingProvider` (this row originally described a documented local no-op, before that provider existed - see `migration_assessment.md`) | Databricks Foundation Model API / model-serving embedding endpoint, via the now fully-implemented `DatabricksEmbeddingProvider` | `LocalEmbeddingProvider`/`OllamaEmbeddingProvider` → `DatabricksEmbeddingProvider` | `embedding.provider: databricks` |
...
| `uvicorn api.main:app` (local process, serving the React build) | Databricks App | The app was originally built with Streamlit specifically so it would deploy as a Databricks App unmodified; it has since been rebuilt as a React frontend + FastAPI backend, so this row needs re-verifying against a FastAPI-on-Databricks-Apps deployment target rather than assumed unmodified. Its provider dependency, `deps.get_agent()`/`deps.get_approval_provider()` etc., already goes through `providers.get_approval_provider(config)` and friends. | n/a, plus optionally `approval.provider: ontobricks` if the review store itself also moves |
